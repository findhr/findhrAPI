...
import os
import pandas as pd
from findhr.fairness.modules.gFair.helpers import train, transform
...
class gFair(PreprocessingFairnessIntervention):
    """
    Class for applying the gFair pre-processing fairness intervention.
    ...
    A_igf : float
        hyperparameter for Ligf, the in-group fairness loss
    ...
    def __init__(self, query_col, feature_cols, score_col, sensitive_col, k, A_x, A_z, A_igf,
                 optimisation_type={"independent": "all"}, model_occ=False, group_weights=None, biggest_gap=False,
                 pos_th=None, maxfun=1000, maxiter=1000, nb_restarts=3, verbose=False, print_interval=100, out_path=None,
                file_name=None, seed=42):
        """
        Initialize the gFair model with the params and creates a configuration dict.
        """
        self.name = "gFair"
