...
from findhr.xai.counterfactual.dice_ml import diverse_counterfactuals as exp
from findhr.xai.counterfactual.dice_ml.constants import ModelTypes
from findhr.xai.counterfactual.dice_ml.explainer_interfaces.explainer_base import ExplainerBase
...
class DiceGenetic(ExplainerBase):
    ...
    def compute_proximity_loss(self, x_hat_unnormalized, query_instance_normalized):
        ...
        for c, f_i in zip(self.data_interface.setlist_feature_names, self.data_interface.setlist_feature_indexes):
            dec_x1 = self.labelencoder[c].inverse_transform(np.atleast_1d(self.x1[f_i]).astype(int))
            dec_cf = self.labelencoder[c].inverse_transform(np.atleast_1d(x_hat[:, f_i]).astype(int))

            proximity_loss += np.array([
                self.jaccard_frozenset(dec_x1[0], cf) for cf in dec_cf]) * self.feature_weights_list[0][f_i]
    ...
    def compute_sparsity_loss(self, cfs):
        ...
        sparsity_loss = np.count_nonzero(self.x1 - cfs, axis=1)
            ...
            if prob < 0.40:
                ...
                if feat_name in features_to_vary:
                    if feat_name in self.data_interface.continuous_feature_names:
                        ...
                        one_init.append(self.feature_range[feat_name][chosen])
                else:
    ...
    def label_encode(self, input_instance):
        for column in self.data_interface.categorical_feature_names + self.data_interface.setlist_feature_names:
            if column in self.data_interface.setlist_feature_names:
                input_instance[column] = input_instance[column].apply(lambda x: frozenset(x))
        ...
        for j in range(num_to_decode):
            ...
            for i in range(len(labelled_input[j])):
                if self.data_interface.feature_names[i] in self.data_interface.categorical_feature_names\
                        + self.data_interface.setlist_feature_names:
    ...
    def get_valid_feature_range(self, normalized=False):
        ...
        for feat_name in self.data_interface.categorical_feature_names + self.data_interface.setlist_feature_names:
            # if feat_name in self.data_interface.setlist_feature_names:
            #     a = np.empty(len(ret[feat_name]), dtype=object)
            #     a[:] = ret[feat_name]
                # ret[feat_name] = [frozenset(self.feature_range[feat_name])]
            #     ret[feat_name] = self.labelencoder[feat_name].transform(a)
            # else:
