...
# Custom LabelEncoder for handling setlist features (in particular to manage the encoding of setlist features with
# a single value to encode)
from findhr.xai.counterfactual.dice_ml.data_interfaces.labelencoder import LabelEncoder
# from sklearn.preprocessing import LabelEncoder

from findhr.xai.counterfactual.dice_ml.data_interfaces.base_data_interface import _BaseData
from findhr.xai.counterfactual.dice_ml.utils.exception import SystemException
...
class PublicData(_BaseData):
    ...
    def get_valid_feature_range(self, feature_range_input, normalized=True):
        ...
        for _, feature_name in enumerate(self.feature_names):
            ...
            if feature_name in self.continuous_feature_names:
                ...
                # setlist features
                feature_range[feature_name] = [frozenset(sorted(x)) for x in self.data_df.loc[:, feature_name].unique()]
            else:
    ...
    def fit_label_encoders(self):
        labelencoders = {}
        for column in self.categorical_feature_names + self.setlist_feature_names:
            labelencoders[column] = LabelEncoder()
            if column in self.categorical_feature_names:
                labelencoders[column] = labelencoders[column].fit(self.data_df[column])
            else:
                labelencoders[column] = labelencoders[column].fit([frozenset(sorted(x)) for x in self.data_df[column]])
